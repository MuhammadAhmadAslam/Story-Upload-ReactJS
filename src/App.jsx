...
function App() {
  ...
  let addStatusFunction = () => {
    if (bgColor && textColor && statusText) {
      setAllStatus([{ bgColor, textColor, statusText, timeRemaining: 30 }, ...allStatus]);
      setBgColor('#35901D');
      setTextColor('#000');
      setStatusText('');
    }
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setAllStatus(allStatus
        .map(status => ({ ...status, timeRemaining: status.timeRemaining - 1 }))
  ...
  return (
    <>
      ...
      <Make
        ...
        addStatusFunction={addStatusFunction}
      />
